...
# residual plot
plt = plot(range(1,stop=nmax), residual, xlabel="n", ylabel="residual", yscale=:log10)
display(plt)
readline()

# countour plot1
plt = plot(x, y, phi', xlabel="x", ylabel="y", label="phi")
display(plt)
readline()

# countour plot2
plt = plot(x, y, va', xlabel="x", ylabel="y", label="sqrt(u^2+v^2)")
display(plt)
readline()
